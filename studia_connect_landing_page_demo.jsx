...
import JourneyClassicTimeline from './components/journey/JourneyClassicTimeline'
import JourneyCurvedRoadmap from './components/journey/JourneyCurvedRoadmap'
import JourneyCircularFlow from './components/journey/JourneyCircularFlow'
import JourneyStickyTimeline from './components/journey/JourneyStickyTimeline'
...
export default function StudiaConnectLandingPage() {
  ...
  // Journey UX Concept Switcher State
  const [activeJourneyLayout, setActiveJourneyLayout] = useState('classic')
      ...
      <main id="main-content">
        ...
        >
          <div className="max-w-7xl mx-auto px-6">
            ...
            {/* Journey UX Concept Switcher */}
            <div className="flex flex-wrap justify-center gap-3 mb-12">
              {[
                { id: 'classic', label: 'Classic Timeline' },
                { id: 'curved', label: 'Curved Roadmap' },
                { id: 'circular', label: 'Circular Flow' },
                { id: 'sticky', label: 'Sticky Timeline' },
              ].map((layout) => (
                <button
                  key={layout.id}
                  onClick={() => setActiveJourneyLayout(layout.id)}
                  className={`relative px-6 py-3 rounded-full text-sm font-semibold transition-colors ${
                    activeJourneyLayout === layout.id
                      ? 'text-white border border-transparent'
                      : 'text-[#082b5f] hover:bg-[#ebe5dc] border border-[#d8c9b4]'
                  }`}
                >
                  {activeJourneyLayout === layout.id && (
                    <motion.div
                      layoutId="activeLayoutTab"
                      className="absolute inset-0 bg-[#082b5f] rounded-full shadow-lg"
            ...
            {/* Render Selected Journey Component */}
            {activeJourneyLayout === 'classic' && <JourneyClassicTimeline services={services} />}
            {activeJourneyLayout === 'curved' && <JourneyCurvedRoadmap services={services} />}
            {activeJourneyLayout === 'circular' && <JourneyCircularFlow services={services} />}
            {activeJourneyLayout === 'sticky' && <JourneyStickyTimeline services={services} />}
          </div>
        </motion.section>
